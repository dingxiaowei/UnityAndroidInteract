...
{
    /// <summary>
    /// DeviceInfoHandler 的摘要说明
    /// </summary>
    public class DeviceInfoHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            else
            {
                ...
                var txt = Path.Combine(basePath, $"{ConstString.DevicePrefix}{testTime}{ConstString.TextExt}");
                if (!File.Exists(txt))
                {
                    context.Response.Write($"error:DeviceInfo:{txt}不存在");
                }
                else
                {
